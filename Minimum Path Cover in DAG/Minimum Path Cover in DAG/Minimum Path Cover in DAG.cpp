...
public:
	...
	MaximumBipartiteMatching(int n) : N(n) {
		adj.resize(N);
		pairs.resize(N);
		seen.resize(N);
		mark = 0;
...
public:
	...
	void print_path() {
		...
		for (int i = 0; i < N; i++) {
			if (!seen[i]) {
				seen[i] = true; int now = i; bool found = true;
				printf("%d", i);
				while (found) {
					found = false;
					for (int j = N; j < 2 * N; j++) {
...
void main() {
	...
	solver.add_edge(7, 5);
	solver.add_edge(7, 6);
	assert(solver.minimum_path_cover() == 3); //verifyig the remaining verticies inside the vector by using assert function.
	printf("\t*.*.*.*Minimum Path Cover in Direct Acyclic Graph*.*.*.*\n");
	solver.print_path();
	_getch();
}